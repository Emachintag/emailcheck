...
def check_whois(domain):
    try:
        ...
        return None  # Whois verisi alınamadıysa None döndür
...
def evaluate_email(email):
    ...
    if whois_check is None:
        print(Fore.RED + "Whois bilgisi alınamadı.")
        uyarilar.append("Whois kontrolünde sorun: Bilgi alınamadı.")
    elif not whois_check:
        print(Fore.RED + "Domain 1 yıldan kısa süredir aktif.")
    ...
    # 6. Türkçe Argo/Küfür Kontrolü:
    print(Fore.BLUE + "Argo/Küfür Kontrolü:")
    slang_found = check_turkish_slang(email, domain, slang_words)
    if slang_found:
        print(Fore.RED + "Argo veya küfür içeriyor.")
        uyarilar.append(f"Argo veya küfür içeriyor: {', '.join(slang_found)}.")
    else:
